...
if (isset($_POST['login'])) {
    ...
    $result = mysqli_query($conn, "SELECT * FROM `users` WHERE User_name='$User_name' AND  Email = '$Email'");
    $row = mysqli_fetch_array($result);

    if ($row) {
       

        // Use password_verify to check if the entered password matches the stored password
        if ($Password) {
            $_SESSION['login'] = "1";
